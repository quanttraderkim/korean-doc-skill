...
`경계값`, `보안 경계`, `API 호출 한도`, `DAU`, `user_id`처럼 이미 자연스럽거나 정확한 전문 용례는 유지한다. 기술 설계나 AI 안전 문서에서 이미 합의한 개념명, 조직 표준 용어, 필드명으로 쓰는 `fallback`·`guardrail`도 유지한다. 단어 목록을 금칙어처럼 적용하지 말고 문맥을 판단한다.

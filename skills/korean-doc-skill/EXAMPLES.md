...
| 협업 기준 문서 | `HANDOFF.md를 정본으로 관리` | `HANDOFF.md를 협업 기준 문서로 사용` |
...
`경계값`, `보안 경계`, `API 호출 한도`, `DAU`, `user_id`처럼 이미 자연스럽거나 정확한 전문 용례는 유지한다. 단어 목록을 금칙어처럼 적용하지 말고 문맥을 판단한다.

## Invented jargon and abbreviations

Avoid:

- `ACPU는 8일 원가를 평일 평균 DAU로 나눈 값`
- `전략 임팩트를 높일 신규 레버 발굴`

Prefer:

- `8일 원가를 평일 평균 DAU로 나눈 비교값` 또는 `활성 사용자 1명당 비용` (산식과 분모 병기)

...
기본 원칙은 세 가지입니다. 첫째, 사용자는 `전략 문서`, `PRD`, `위클리`, `이슈 대응`, `허브 문서`처럼 대략적인 문서 성격만 말하면 됩니다. 둘째, 독자와 이 문서를 읽고 내려야 할 판단·다음 행동이 맥락에서 분명하면 에이전트가 이를 먼저 반영합니다. 셋째, mode는 특별한 이유가 없으면 생략합니다. mode를 생략하면 스킬은 기본적으로 원문 논리를 유지하되 독자에게 필요 없는 중복과 실무 세부를 덜어내는 쪽으로 해석합니다.
...
2. 원문 초안을 함께 줌
3. `전략 문서`, `PRD`, `위클리`, `이슈 대응`, `허브 문서`처럼 대략적인 문서 성격과 독자를 말함
4. 이 문서를 읽고 내려야 할 판단이나 다음 행동이 있으면 함께 말함
...
- 독자와 필요한 판단을 먼저 정하고, 리더·임원용 보고에는 그 판단에 필요한 현황·의미·제안·결정사항만 유지
- 간결의 정의: 빈말·중복·불필요한 실무 세부를 걷어내고 판단을 뒷받침하는 대표 근거는 유지
- 문서 성격에 맞춰 정보 밀도 조절: 전략·기획·분석·보고·제안·킥오프도 간결한 보고체가 기본이며, 런북·체크리스트·위클리는 같은 원칙을 더 압축
- 라벨 섹션(`결론`, `요약`, `주의점`, `다음 액션` 류)은 불릿 3개 이내
- 불릿 한 개는 한 포인트. 화면 폭에 따른 줄바꿈은 허용하되 독립적인 판단이 둘이면 분리
- 섹션은 메뉴. 내용 없는 섹션은 템플릿 채우기식으로 만들지 않음
- 재작성은 확장 요청이나 빠진 근거 보완이 없으면 원문보다 길어지지 않는 것이 기본
- 에세이형 설명보다 보고용 메모체 우선
- `~입니다 / ~합니다 / ~됩니다` 최소화
- 모든 문장을 기계적으로 `~함`으로 끝내지는 않음
- 긴 설명형 `~한다`체도 기본값으로 두지 않음
- 연결 논리만 2문장 이내의 짧은 문단으로 두고, 독립적인 판단·수치·근거·조건·액션은 불릿으로 표시
- 명사형, 짧은 판단형 끝맺음 우선
...
- 기본 규칙, 예외 처리, 우선 적용 규칙, 변경 이력은 한 문장으로 합치지 않고 불릿 계층으로 분리
- 표는 비교 축, 반복 필드, 소유자, 리스크, 옵션 정리가 있을 때만 사용
- 실제 열이 있어도 독자의 비교·결정을 더 빠르게 하지 않으면 표를 사용하지 않고, 같은 질문에 답하는 표는 합치거나 삭제
...
- 펼치기는 중복·불필요한 세부 보관용이 아니며, 검증이나 질의 대응에 필요한 근거만 유지
- `먼저 볼 항목`, `지금 먼저 볼 것`, `읽기 순서`, `권장 읽기 순서` 같은 메타 제목은 기본값으로 쓰지 않음
- 병렬 항목 자체가 내용인 단순 리스트는 불릿으로 유지하고, 모든 불릿을 제목으로 승격하지 않음
- 영어 단어를 먼저 정한 뒤 한국어 하나를 고정 대응하지 않고 문장에서 하는 역할에 맞춰 표현
- 일반 문장의 `정본`, `경계값`, `보안 경계`, `오너`, `드라이버`처럼 직역한 표현은 기준 문서·판정 기준·망 분리 구간·담당자·주요 요인처럼 실제 역할이 드러나는 말로 변경. 정확한 필드명이나 법령·사내 규정의 공식 명칭은 유지
- DAU·MAU·API·MCP·제품명·필드명은 유지하고 통용되지 않는 새 약어는 만들지 않음

...
기본 사용 상황은 사용자가 읽을 답변을 만드는 것이 아니라, 사용자가 작성자·편집자가 되어 다른 사람에게 공유하거나 발표할 문서를 만드는 것입니다. 파일이 Markdown이라는 이유만으로 적용하지 않습니다. 요청자는 보통 독자가 아니며, Confluence 본문에는 에이전트의 작업 과정이나 작성자에게 보내는 진행 보고가 아니라 실제 독자에게 필요한 판단·근거·정책·절차만 남깁니다. 결과 해석을 바꾸는 한계와 독자가 직접 써야 하는 미결 정책·중단 조건은 예외로 보존합니다.
...
Symlink한 checkout의 변경은 즉시 공용 스킬에 반영되므로 그 checkout은 `main`에만 두고, feature branch는 별도 git worktree에서 작업한 뒤 병합 후 별도 단계에서 최신 `main`으로 갱신합니다.
